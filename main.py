...
class WdttApp(ctk.CTk):
    ...
    def _build_tab_settings(self):
        ...
        ctk.CTkLabel(c1_inner, text="ПАРАМЕТРЫ VK TURN (CONFIG.INI)", font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold"), text_color=COLOR_ACCENT).pack(anchor="w", pady=(0, 12))
        ...
        # VK Hash Key
        ctk.CTkLabel(c1_inner, text="Хеш VK-звонка / Ключ подключения (VK_HASH):", font=ctk.CTkFont(family="Segoe UI", size=11), text_color=COLOR_TEXT_MUTED).pack(anchor="w")
        self.entry_vkhash = ctk.CTkEntry(c1_inner, fg_color="#0D111A", border_color=COLOR_CARD_BORDER, text_color=COLOR_TEXT_PRIMARY, height=36)
        self.entry_vkhash.insert(0, self.config_mgr.vk_hash)
        self.entry_vkhash.pack(fill="x", pady=(2, 10))
        ...
        # Card: App Settings
        card2 = ctk.CTkFrame(page, fg_color=COLOR_CARD, corner_radius=14, border_width=1, border_color=COLOR_CARD_BORDER)
        card2.pack(fill="x", pady=(0, 12))
        c2_inner = ctk.CTkFrame(card2, fg_color="transparent")
        c2_inner.pack(fill="x", padx=20, pady=16)

        ctk.CTkLabel(c2_inner, text="ПОВЕДЕНИЕ ПРИЛОЖЕНИЯ И СЕТЬ", font=ctk.CTkFont(family="Segoe UI", size=13, weight="bold"), text_color=COLOR_ACCENT).pack(anchor="w", pady=(0, 12))

        # PCVPN Port
        ctk.CTkLabel(c2_inner, text="Порт прокси для телефона (PCVPN):", font=ctk.CTkFont(family="Segoe UI", size=11), text_color=COLOR_TEXT_MUTED).pack(anchor="w")
        self.entry_pcvpn_port = ctk.CTkEntry(c2_inner, fg_color="#0D111A", border_color=COLOR_CARD_BORDER, text_color=COLOR_TEXT_PRIMARY, height=36)
        ...
        ctk.CTkLabel(c2_inner, text="Действие при нажатии на крестик [X]:", font=ctk.CTkFont(family="Segoe UI", size=11), text_color=COLOR_TEXT_MUTED).pack(anchor="w")
        ...
        # DNS Mode
        ctk.CTkLabel(c2_inner, text="DNS для VK:", font=ctk.CTkFont(family="Segoe UI", size=11), text_color=COLOR_TEXT_MUTED).pack(anchor="w")
        ...
        self.combo_dns.set(self.config_mgr.settings.get("go_dns", "yandex"))
        self.combo_dns.pack(fill="x", pady=(2, 15))
        ...
        btn_make_shortcut.pack(fill="x", pady=(0, 5))
    ...
    def _save_all_settings(self):
        ...
        vk_hash = self.entry_vkhash.get().strip()
        workers = self.entry_workers.get().strip()
        pcvpn_port_str = self.entry_pcvpn_port.get().strip()

        # Update config.ini
        self.config_mgr.save_ini(peer=peer, password=password, vk_hash=vk_hash, workers=workers)
        ...
        self.key_entry.insert(0, vk_hash)
